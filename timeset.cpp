...
class timer
{
    ...
    public:
        ...
        int mark()
        {
            ...
            auto duration = chrono::duration_cast<chrono::milliseconds>(now - checkpoint).count();
            checkpoint = now;
            return duration;
        }
        int total()
...
int cordtoidx(const struct ImgSize& imgSize, const struct coord& c){
 ...
 */
BYTE * loadfile(const string filename, struct ImgSize& imgSize,BITMAPFILEHEADER* bmpFHeader, BITMAPINFOHEADER * bmpIHeader )
{
 ...
 */
void savefile(const string filename, const struct ImgSize imgSize, const BITMAPFILEHEADER bmpFHeader, const BITMAPINFOHEADER bmpIHeader, const BYTE* imgData)
{
 ...
 * @see masking
 */
BYTE * makeQuadMask(struct ImgSize imgSize, const coord * maskingCords)
{
 ...
 * @see makeQuadMask
 */
BYTE * masking(const BYTE * source, const BYTE * mask, struct ImgSize imgSize)
{
...
int main() {
    ...
    cout << "Time taken for loading images: " << t.mark() << " ms" << endl;
    ...
    cout << "Time taken for camera parameter homography: " << t.mark() << " ms" << endl;
    ...
    cout << "Time taken for ORB feature detection and matching: " << t.mark() << " ms" << endl;
    ...
    cout << "Time taken for ratio test and homography computation: " << t.mark() << " ms" << endl;
    ...
    cout << "Time taken for OpenCV homography transformation: " << t.mark() << " ms" << endl;
    ...
    cout << "Time taken for specification and comparison: " << t.mark() << " ms" << endl;
    cout << "RMSE between images: "<< rmse(comp_result, secondCV) << endl;
    cout << "IOU between masks: " << iou(homographyCords, homographyCordsCV) << endl;
    //최종 결과를 second.bmp의 마스킹에 적용하여 저장
    auto final_masked = masking(simgData, makeQuadMask(simgSize, homographyCords), simgSize);
    savefile("output/final_masked_output.bmp", simgSize, sbmpFHeader, sbmpIHeader, final_masked);
    //종료 시간
    cout << "Execution time: " << t.total() << " ms" << endl;
